...
machineInventory = {
    ...
    'IM': {'Description': 'Iced Milo', 'Price': 1.5, 'Quantity': 50},
    'HM': {'Description': 'Hot Milo', 'Price': 1.2, 'Quantity': 50},
    'IC': {'Description': 'Iced Coffee', 'Price': 1.5, 'Quantity': 50},
    'HC': {'Description': 'Hot Coffee', 'Price': 1.2, 'Quantity': 50},
    '1P': {'Description': '100 Plus', 'Price': 1.1, 'Quantity': 30},
    'CC': {'Description': 'Coca Cola', 'Price': 1.3, 'Quantity': 20},
...
def display_drinks():
    for key, value in machineInventory.items():
        desc2 = (str(key) + " : " + str(value.get('Description')))
        lol = "($" + str(value.get('Price')) + ")"
        test = str(desc2) + "" + str(lol)
        align_left = test.ljust(25)  # Forces it so that no matter what its 25 spaces from the left, so as to align the Quantities
        if value.get('Quantity') == 0:
            print(align_left, "Qty: *** Out Of Stock ***")
        else:
            print(align_left, "Qty:" + str(value.get('Quantity')))
...
# function to check if number (quantity inputted) is valid, for those that cannot be 0
...
# function to check if number (quantity inputted) is valid
